...
export const Navbar = () => {
  ...
  const [ownedByHr, setOwnedByHr] = useState('');
  const [useArConnect, setUseArConnect] = useState(false);
  const [price, setPrice] = useState(0);

  useEffect(() => {
    console.log('TYPE OF UUID ', typeof uuidv4());
  ...
  async function writeToFeed(keys, contractId, id) {
    ...
    console.log('WRF CONTRACT ID', contractId);
  ...
  async function writeTransaction(buffer) {
    ...
    if (!useArConnect) {
      console.log('NON ARCONNECT CONDITION');
      ...
      console.log('ENTERED USE ARCONNECT CONDITION');
      let transactionA = await arweave.createTransaction({
        data: buffer,
      });
            ...
            <Fade in={postImageModalIsOpen}>
              <div className={classes.paper}>
                ...
                <TextField
                  ...
                  label="Owned By"
                  variant="outlined"
                  onChange={(e) => setOwnedByHr(e.target.value)}
                ...
                <FormControlLabel
                  control={
                    <Checkbox
                      checked={useArConnect}
                      onChange={() => setUseArConnect(!useArConnect)}
                      name="checkedB"
                      color="primary"
                    />
                  }
