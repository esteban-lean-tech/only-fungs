...
export default function NFT({
  ...
  const [futureOwnedByHr, setFutureOwnedByHr] = useState('');
  ...
  async function purchaseNFT(owner, id, contractId) {
    ...
    const input = {
      id: id,
      function: 'buy',
      ownedByHR: futureOwnedByHr,
    };

    console.log('INPUT IN PURCHASE NFT', input);
    ...
    console.log('TRANSACTION BEFORE SIGNING IN PURCHASE', transaction);
    ...
    console.log('TRANSACTION AFTER SIGNING IN PURCHASE ', transaction);

    console.log(id, 'NFTID');
    const txid = await interactWrite(arweave, wallet, contractId, input);
    console.log('TX ID ', txid);

    Swal.fire('Good job!', 'You purchased NFT from ' + owner, 'success');
  }
      ...
      <CardActionArea>
        <CardContent>
          <Box flexDirection="row" display="flex">
            <div>
              ...
              <Typography gutterBottom>
                <strong>CURRENT OWNER: {ownedByHr} </strong>
                <p>CURRENT OWNER PUBLIC KEY: {publicKey}</p>
              </Typography>
            </div>
            <div className={classes.likeButtonsDiv}>
              {nftLiked ? (
                <Button
                  size="small"
                  className={classes.likeButtonGreen}
                  onClick={() => {
                    removeLike();
                  }}
                >
              ...
              {nftDisliked ? (
                <Button
                  size="small"
                  className={classes.dislikeButtonRed}
                  onClick={() => {
                    removeDislike();
                  }}
                >
        ...
        <CardContent>
          <Typography variant="body2" color="textSecondary" component="p">
            ...
            >
              <path
                d="M5.42553 0L3.8883 9.63636H6.05851V19H0V9.63636L2.26064 0H5.42553ZM16.367 0L14.8298 9.63636H17V19H10.9415V9.63636L13.2021 0H16.367Z"
                fill="#8A939B"
                fill-opacity="0.25"
              />
            </svg>
          ...
          <Box flexDirection="row" display="flex">
            <Box p={1}>
              ...
              <Typography gutterBottom className={classes.priceText}>
                <span className={classes.bluePriceText}>$0.12</span> / .01 ⓐ
              </Typography>
            </Box>
            <Box>
              <CardActions>
                <TextField
                  ...
                  label="Your Name"
                  variant="outlined"
                  onChange={(e) => setFutureOwnedByHr(e.target.value)}
                />
                ...
                <Button
                  ...
                  onClick={() => purchaseNFT(publicKey, id, contractId)}
                >
                  Buy Now
                </Button>
              </CardActions>
